...
def main():
    ...
    from document_processor import FixedSizeTextSplitter, add_pymupdf_page_metadata, Document
...
def main():
    ...
    parser.add_argument("--worker-batch-size", type=int, default=2000,
                        help="Number of DOCUMENTS to give each worker subprocess (default: 2000). "
                             "Each worker process dies after this many docs, getting a fresh heap.")
    parser.add_argument("--max-worker-retries", type=int, default=3,
                        help="Max retries for a single worker chunk before skipping (default: 3)")
    parser.add_argument("--max-parallel-workers", type=int, default=0,
                        help="Max parallel worker subprocesses (default: 0 = auto, "
                             "uses physical_cores - 2). Set to 1 for sequential.")
    parser.add_argument("--checkpoint-dir", type=Path, default=None)
    parser.add_argument("--checkpoint-interval", type=int, default=5,
                        help="Save checkpoint every N worker completions (default: 5)")
    ...
    logger.info(f"  parallel_workers={effective_parallel} "
                ...
                f"min_docs_for_parallel={MIN_DOCS_FOR_PARALLEL})")
    t0 = time.time()
    ...
    logger.info(f"Processing {total_docs} documents in {total_workers} worker subprocess(es) "
                f"({worker_batch_size} docs each, up to {effective_parallel} in parallel)")
    ...
    if effective_parallel <= 1:
        for wid, chunk_docs in worker_jobs:
            ...
            if checkpoint_path is not None and workers_since_checkpoint >= args.checkpoint_interval:
                ...
                workers_since_checkpoint = 0
                logger.info(f"  Checkpoint saved ({len(all_texts)} chunks so far)")
                ...
                for future in concurrent.futures.as_completed(future_to_wid):
                    ...
                    try:
                        ...
                        wave_results[wid] = {
                            ...
                            "texts": [],
                            "chunks": [],
                            "errors": [{"doc_index": -1, "file_name": "THREAD_EXCEPTION",
    ...
    logger.info(f"Split {total_docs} documents into {len(all_texts)} chunks in {elapsed:.1f}s "
                ...
                f"{workers_completed} workers, parallel={effective_parallel})")

    if all_errors:
        logger.warning(f"Split errors:")
        for err in all_errors[:20]:
            logger.warning(f"  doc {err['doc_index']}: {err['file_name']} - {err['error']}")
        if len(all_errors) > 20:
            logger.warning(f"  ... and {len(all_errors) - 20} more")
