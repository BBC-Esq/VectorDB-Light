...
class ChunkSettingsTab(QWidget):
    def __init__(self):
        ...
        self.compute_device_options = config.Compute_Device.available
        self.database_creation_device = config.Compute_Device.database_creation
        ...
        current_size = self.database_config.chunk_size
        ...
        current_overlap = self.database_config.chunk_overlap
        ...
        self.half_precision_checkbox.setChecked(self.database_config.half)
    ...
    def update_config(self):
        ...
        if new_device != self.database_creation_device:
            success, msg = config.update_setting("Compute_Device.database_creation", new_device)
            if success:
        ...
        if new_chunk_size_text:
            try:
                ...
                if new_chunk_size <= 0:
                    ...
                    success, msg = config.update_setting("database.chunk_size", new_chunk_size)
                    if success:
        ...
        if new_chunk_overlap_text:
            try:
                ...
                if new_chunk_overlap < 0:
                    ...
                    current_size = config.database.chunk_size
                    if new_chunk_overlap >= current_size:
                        errors.append("Chunk overlap must be less than chunk size.")
                    else:
                        success, msg = config.update_setting("database.chunk_overlap", new_chunk_overlap)
                        if success:
