...
def run_worker(python_exe: str, worker_script_path: str,
               ...
               timeout: int = 600) -> tuple[int, float]:
    cmd = [
...
def run_worker_with_retries(worker_id: int, total_workers: int,
                            ...
                            chunk_texts: list, global_start: int,
                            worker_dir: Path, model_path: str,
                            batch_size: int, max_seq_length: int,
                            use_fast: bool, max_retries: int,
                            encode_batch_size: int, length_sort: bool) -> dict:
        ...
        if exit_code == 0 and result_pkl.exists():
            try:
                ...
                logger.info(f"  Worker {worker_id}/{total_workers} completed in {elapsed:.1f}s "
                           f"({len(batches)} batches, {len(errors)} errors, "
                           f"{eff:.1f}% pad efficiency)")
                break
            except Exception as e:
    ...
    if not worker_success:
        logger.error(f"  Worker {worker_id} FAILED after {max_retries} retries, "
                    f"skipping texts {global_start} to {global_start + num_texts - 1}")
...
def main():
    ...
    logger.info(f"  use_fast={args.use_fast}, worker_batch_size={args.worker_batch_size}")
    logger.info(f"  start_text_index={start_text_index}")
    logger.info(f"  parallel_workers={effective_parallel} "
                f"(physical_cores={get_physical_core_count()}, "
                f"min_texts_for_parallel={MIN_TEXTS_FOR_PARALLEL})")
    t0 = time.time()

    logger.info(f"Loaded {total_all} texts from {args.input_pickle}")
    ...
    logger.info(f"Processing {total} texts in {total_workers} worker subprocess(es) "
                f"({worker_batch_size} texts each, up to {effective_parallel} in parallel)")
    ...
    if effective_parallel <= 1:
        for wid, gstart, chunk in worker_jobs:
            logger.info(f"Worker {wid}/{total_workers}: texts {gstart} to "
                        f"{gstart + len(chunk) - 1} ({len(chunk)} texts)")
            ...
            if checkpoint_path is not None and workers_since_checkpoint >= checkpoint_interval:
                ...
                workers_since_checkpoint = 0
                logger.info(f"  Checkpoint saved (text index {start_text_index + current_offset})")
                ...
                for future in concurrent.futures.as_completed(future_to_wid):
                    ...
                    try:
                        ...
                        wave_results[wid] = {
                            ...
                            "success": False,
                            "batches": [],
                            "errors": [{"batch_id": -1, "start_index": -1,
                                       "error": f"Thread exception: {e}"}],
                            "padding_stats": {},
                        }

            for wid, gstart, chunk in wave_jobs:
                result = wave_results.get(wid, {
                    "batches": [], "errors": [],
                    "num_texts": len(chunk), "padding_stats": {},
                })
                all_batches.extend(result["batches"])
                all_errors.extend(result["errors"])
                ps = result.get("padding_stats", {})
    ...
    logger.info(f"Tokenized {len(all_batches)} batches in {elapsed:.1f}s "
                f"({len(all_errors)} errors, {workers_completed} workers, "
                f"parallel={effective_parallel})")
    logger.info(f"  Padding efficiency: {efficiency_pct:.1f}% "
                f"({total_real_tokens:,} real, {total_pad_tokens:,} padding)")
    ...
    file_size_mb = args.output_pickle.stat().st_size / (1024 * 1024)
    logger.info(f"Wrote {args.output_pickle} ({file_size_mb:.1f} MB)")
